...
- [Chat UI & jathakam page layout — visual and interaction design](tickets/015-chat-ui-jathakam-layout.md) — jathakam: single vertical scroll (chart hero → panchanga "at birth" → dasha), mahadasha list with tap-to-expand antardasha on the current period only, sticky disclaimer footer. Chat: inline tap-for-grounding citations (in-flow reveal, not a popover), token-by-token streaming, "AI" marker in the header subtitle, violet send button. App-wide rule locked: serif for occasional/ritual content, sans for anything frequent or data-dense. Asset: [`spikes/chat-jathakam-prototype/`](../../../spikes/chat-jathakam-prototype/README.md). No UI/UX fog remains open on this map; unblocks [Assemble the engineering spec](tickets/012-assemble-spec.md) (jointly with the map's other open tickets).
